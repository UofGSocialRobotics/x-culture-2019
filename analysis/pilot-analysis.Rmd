...
bucket_culture_scores <- function(dat) {
  d <- dat %>%
    mutate(score_bucket = ifelse(score >= 5, "ahigh", ifelse(score <= 2, "clow", "bmedium")))
  return(d)
}
...
get_bucketed_data <- function(dat) {
  ...
  for(g in groups) {
    d <- bucket_culture_scores(dat) %>%
        ...
        group_by(viewer_culture, condition) %>%
        mutate(bucket_percent = count_bucket / sum(count_bucket)) %>%
...
test_bucketed_significance <- function(dat) {
  ...
  df <- data.frame(condition, group, high_comp, med_comp, low_comp)
  return(df)
}
...
wg1_diffrence_table <- test_bucketed_significance(wg1_summary_stats)
wg2_diffrence_table <- test_bucketed_significance(wg2_summary_stats)
wg3_diffrence_table <- test_bucketed_significance(wg3_summary_stats)
wg4_diffrence_table <- test_bucketed_significance(wg4_summary_stats)
wg5_diffrence_table <- test_bucketed_significance(wg5_summary_stats)

eg1_diffrence_table <- test_bucketed_significance(eg1_summary_stats)
eg2_diffrence_table <- test_bucketed_significance(eg2_summary_stats)
eg3_diffrence_table <- test_bucketed_significance(eg3_summary_stats)
eg4_diffrence_table <- test_bucketed_significance(eg4_summary_stats)
```
