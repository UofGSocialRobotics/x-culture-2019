...
w_gests_correlational.w <- rbind(create_difference_matrix(wg1.0.w), 
...
w_gests_correlational.e <- rbind(create_difference_matrix(wg1.0.e), 
...
e_gests_correlational.w <- rbind(create_difference_matrix(eg1.0.w),
...
e_gests_correlational.e <- rbind(create_difference_matrix(eg1.0.e),
...
all_western_correlational_data.w <- plot_diff_matrix_pretty(w_gests_correlational.w, 
    "Correlation of questions across all gesture conditions for western participants")

all_western_correlational_data.e <- plot_diff_matrix_pretty(w_gests_correlational.e, 
    "Correlation of questions across all gesture conditions for western participants") 

all_eastern_correlational_data.w <- plot_diff_matrix_pretty(e_gests_correlational.w, 
    "Correlation of questions across all gesture conditions for western participants")

all_eastern_correlational_data.e <- plot_diff_matrix_pretty(e_gests_correlational.e, 
    "Correlation of questions across all gesture conditions for eastern participants") 

all_correlational_data.w <- plot_diff_matrix_pretty(rbind(w_gests_correlational.w, e_gests_correlational.w)) 
all_correlational_data.e <- plot_diff_matrix_pretty(rbind(w_gests_correlational.e, e_gests_correlational.e))

all_correlational_data.we <- plot_diff_matrix_pretty(rbind(w_gests_correlational.w, e_gests_correlational.w,
                                                           w_gests_correlational.e, e_gests_correlational.e)) 
...
grid.arrange(all_correlational_data.w,
             all_correlational_data.e)

corr_list <- list(all_correlational_data.w,
               all_correlational_data.e)
ggsave("question_correlations.png", arrangeGrob(grobs = corr_list, ncol=2))

ggsave("question_correlations_combined.pdf", plot=all_correlational_data.we)
```
...
do_t_test_adjusted <- function(cond1, cond2, statement, dat) {
  ...
  p_adjusted <- round(p.adjust(p, method="bonferroni", n=540), digits=5)
  return(p_adjusted)
}
...
make_table_per_gesture_condition_by_culture <- function(dat) {
  groups <- c("openness",
            "conflict",
            "control", 
            "size",
            "certainty",
            "unity")
  conditions <- c("cond1", "cond2", "original")
  western_mean <- c()
  eastern_mean <- c()
  p <- c()
  gs <- c()
  ...
  for(g in groups) {
    for(co in conditions) {
      ...
      cons <- append(cons, co)
      w_only <- dat %>%
        filter(viewer_culture=="western", group==g, condition==co)
      e_only <- dat %>%
        filter(viewer_culture=="eastern", group==g, condition==co)
  ...
  df <- data.frame(gs, cons, western_mean, eastern_mean, p, sig)   
  return(df)
} 


do_t_culture_groups <- function(dat, cond1="cond1", cond2="cond2", gesture_name="unknown") {
  c0 <- make_table_per_gesture_condition_by_culture(dat)
  c1 <- make_table_per_gesture_condition_by_culture(dat)
  c2 <- make_table_per_gesture_condition_by_culture(dat)
...
Basically, "were there significant differences between western and eastern viewers in this set of conditions?"
```{r wg1_violin}
...
bucket_culture_scores <- function(dat) {
  d <- dat %>%
    mutate(score_bucket = ifelse(score >= 5, "ahigh", ifelse(score <= 3, "clow", "bmedium")))
  return(d)
}
...
create_bucketed_bar_chart <- function(dat, gest) {
  ...
  test_comparisons <- list(c("ahigh_eastern","ahigh_western"),c("bmedium_eastern", "bmedium_western"),c("clow_eastern", "clow_western"))
  p <- ggplot(d,aes(x=condition,y=bucket_percent,fill=factor(culture_bucket)))+
    geom_bar(stat="identity",position="dodge")+
    scale_fill_manual("Viewer Culture Score Bucket", 
                      labels=c("Eastern 5-7", "Western 5-7", "Eastern 4", "Western 4", "Eastern 1-3", "Western 1-3"),
                      values=c("ahigh_eastern" = "palegreen3",  # rosybrown1, 3, 4
...
### GOING OFF THE RAILS OF THE CRAZY TRAIN BABY
# create_bucketed_bar_chart_INVERSE <- function(dat, gest) {
#   d <- get_bucketed_data(dat)
#   test_comparisons <- list(c("ahigh_eastern","ahigh_western"),c("bmedium_eastern", "bmedium_western"),c("clow_eastern", # "clow_western"))
#   p <- ggplot(d,aes(x=group,y=bucket_percent,fill=factor(culture_bucket)))+
#     geom_bar(stat="identity",position="dodge")+
#     stat_compare_means(comparisons = test_comparisons, label = "p.signif", method="kruskal.test") +
#     scale_fill_manual(values=c("ahigh_eastern" = "palegreen3",  # rosybrown1, 3, 4
#                                          "bmedium_eastern" = "steelblue2",
#                                          "clow_eastern" = "palevioletred2",
#                                          "ahigh_western" = "palegreen4", # steelblue1, 3, 4
#                                          "bmedium_western" = "steelblue4",
...
#   return(p)
# }
# t <- create_bucketed_bar_chart_INVERSE(wg1_total, "WG1")
###
...
get_summary_stats <- function(dat) {?pwr.anova.test
...
test_bucketed_significance <- function(dat) {
  ...
  for(c in cs) { 
    ...
    for(g in groups) {
      ...
      western_avg <- list.append(western_avg, w$avg)
      eastern_avg <- list.append(eastern_avg, e$avg)
      cult <- "eastern"
      if(w$avg > e$avg) {
        cult <- "western"
      }
      w_high <- list.append(w_high, w$ahigh)
      e_high <- list.append(e_high, e$ahigh)
      w_med <- list.append(w_med, w$bmedium)
      e_med <- list.append(e_med, e$bmedium)
      w_low <- list.append(w_low, w$clow)
      e_low <- list.append(e_low, e$clow)
      higher <- list.append(higher, cult)
  ...
  df <- data.frame(condition, group, high_comp, adjusted_high_comp, med_comp, adjusted_med_comp, low_comp, adjusted_low_comp, w_high, e_high, w_med, e_med, w_low, e_low) %>%
    group_by(group)
  return(df)
}
...
```{r}
all_w_dat <- rbind(wg1_total, wg2_total, wg3_total, wg4_total, wg5_total)
all_e_dat <- rbind(eg1_total, eg2_total, eg3_total, eg4_total)
all_dat <- rbind(all_w_dat, all_e_dat)

all_w_viewer_dat <- all_dat %>%
  filter(viewer_culture == "western")

all_e_viewer_dat <- all_dat %>%
  filter(viewer_culture == "eastern")

num_w_high_responses <- nrow(filter(all_w_viewer_dat, score==7))
num_w_med_responses <- nrow(filter(all_w_viewer_dat, score==4))
num_w_low_responses <- nrow(filter(all_w_viewer_dat, score==1))
num_e_high_responses <- nrow(filter(all_e_viewer_dat, score==7))
num_e_med_responses <- nrow(filter(all_e_viewer_dat, score==4))
num_e_low_responses <- nrow(filter(all_e_viewer_dat, score==1))

prop_w_high <- num_w_high_responses / nrow(all_w_viewer_dat)
prop_w_med <- num_w_med_responses / nrow(all_w_viewer_dat)
prop_w_low <- num_w_low_responses / nrow(all_w_viewer_dat)
prop_e_high <- num_e_high_responses / nrow(all_e_viewer_dat)
prop_e_med <- num_e_med_responses / nrow(all_e_viewer_dat)
prop_e_low <- num_e_low_responses / nrow(all_e_viewer_dat)

w_exp <- 1/7
e_exp <- 1/7

w_stdev <- summarise(all_w_viewer_dat, std = sd(score))[[1]]
e_stdev <- summarise(all_e_viewer_dat, std = sd(score))[[1]]
```
...
```{r}
w_group_avg <- all_w_viewer_dat %>%
  group_by(group) %>%
  summarise(avg=mean(score))

e_group_avg <- all_e_viewer_dat %>%
  group_by(group) %>%
  summarise(avg=mean(score))
```
A: No. Actually, western avg for conflict = 3.94, whereas eastern = 3.97.

Q: In what categories did the differences occur? 
```{r}
count_all_differences <- function(dat){
  diffs_only <- dat %>%
    group_by(group) %>%
    summarise(sig_high_diffs=sum(adjusted_high_comp!=""), sig_med_diffs=sum(adjusted_med_comp!=""), 
              sig_low_diffs=sum(adjusted_low_comp!="")) %>%
    mutate(total=rowSums(.[2:4]))
  return(diffs_only)
}

all_diff_tables <- rbind(wg1_difference_table, wg2_difference_table, wg3_difference_table, wg4_difference_table, 
                         wg5_difference_table, eg1_difference_table, eg2_difference_table, eg3_difference_table,
                         eg4_difference_table)

diff_counts <- count_all_differences(all_diff_tables)
kable(diff_counts)
```
A: `{r}diff_counts`
...
```{r}
all_w_diffs <- rbind(wg1_difference_table, wg2_difference_table, wg3_difference_table, wg4_difference_table, 
                         wg5_difference_table) 
all_e_diffs <- rbind(eg1_difference_table, eg2_difference_table, eg3_difference_table, eg4_difference_table)

total_w_diffs <- count_all_differences(all_w_diffs)
total_e_diffs <- count_all_differences(all_e_diffs)
```
total differences in W gestures: `{r}sum(total_w_diffs$total)`
total differences in E gestures: `{r}sum(total_e_diffs$total)`

Q: But who was higher? 
```
wg1j <- left_join(wg1_difference_table, wg1_summary_stats, by=c("condition" = "condition", "group" = "group"))
wg2j <- left_join(wg2_difference_table, wg2_summary_stats, by=c("condition" = "condition", "group" = "group"))
wg3j <- left_join(wg3_difference_table, wg3_summary_stats, by=c("condition" = "condition", "group" = "group"))
wg4j <- left_join(wg4_difference_table, wg4_summary_stats, by=c("condition" = "condition", "group" = "group"))
wg5j <- left_join(wg5_difference_table, wg5_summary_stats, by=c("condition" = "condition", "group" = "group"))
eg1j <- left_join(eg1_difference_table, eg1_summary_stats, by=c("condition" = "condition", "group" = "group"))
eg2j <- left_join(eg2_difference_table, eg2_summary_stats, by=c("condition" = "condition", "group" = "group"))
eg3j <- left_join(eg3_difference_table, eg3_summary_stats, by=c("condition" = "condition", "group" = "group"))
eg4j <- left_join(eg4_difference_table, eg4_summary_stats, by=c("condition" = "condition", "group" = "group"))
...
j <- rbind(wg1j,
           ...
           eg3j,
           eg4j)
toDelete <- seq(0, nrow(j), 2)
toDelete
n <-  j[-toDelete, ]

diffs_only <- n %>%
  filter((nchar(as.character(adjusted_high_comp)) + 
         nchar(as.character(adjusted_low_comp)) + 
         nchar(as.character(adjusted_med_comp))) != 0)

c <- diffs_only %>%
  group_by(group) %>%
  summarise(w_high_culture=sum(higher=="western"), e_high_culture=sum(higher=="eastern"))

# cert <- diffs_only %>%
  filter(group=="certainty")
```
tbh I didn't figure out how to automate, just did this and counted by hand

this doesn't make any sense though -- our western viewers are always higher than our eastern basically, but they don't show significant ARS or ERS... 

I guess I'll try normalizing results anyway?
```{r}
t <- wg1_total %>%
  filter(viewer_culture == "western") %>%
  group_by(group)
...
```{r}
compare_dists <- function(d1, d2, gesture="") {
  w <- d1 %>%
    summarise('1'=sum(score==1), '2'=sum(score==2), '3'=sum(score==3), 
              '4'=sum(score==4), '5'=sum(score==5), '6'=sum(score==6), '7'=sum(score==7)) %>%
    gather("score_cat", "count", 1:7) %>%
    mutate(viewer_culture="western") 
  ...
  e <- d2 %>%
    summarise('1'=sum(score==1), '2'=sum(score==2), '3'=sum(score==3), 
              '4'=sum(score==4), '5'=sum(score==5), '6'=sum(score==6), '7'=sum(score==7)) %>%
    gather("score_cat", "count", 1:7) %>%
    mutate(viewer_culture="eastern")
  
  difference <- w$count - e$count
  diff_df <- data.frame(score_cat=seq(1,7), count=difference, viewer_culture="difference")
  wed <- rbind(w,e,diff_df) 
    
  g <- ggplot(wed, aes(x=score_cat, y=count, fill=viewer_culture)) + 
    geom_bar(stat="identity", position="dodge") + 
    ggtitle(paste("Overall Score Distributions ", gesture)) +
    theme(text=element_text(size=12, family="serif")) +
    scale_fill_manual("Viewer Culture", 
                  labels=c("Difference", "Eastern", "Western"),
                  values=c("eastern" = "palegreen3",  # rosybrown1, 3, 4
                           "western" = "steelblue2",
                           "difference" = "palevioletred2")) +
    labs(x="Score", y="Score Frequency")
  
  return(g)
}

overall_distribution <- compare_dists(all_w_dat, all_e_dat) 
ggsave("overall_distribution.png", overall_distribution)


## now again but with violin
compare_dists_violin <- function(d1, d2, gesture="") {
  w <- d1 %>%
    mutate(viewer_culture="western") 
  e <- d2 %>%
    mutate(viewer_culture="eastern")
  ...
  wed <- rbind(w,e) 
  g <- ggplot(wed, aes(x=viewer_culture, y=score)) + 
    geom_dotplot(aes(fill=viewer_culture, alpha=0.9), 
                position="identity",
                stackdir='center',
                binaxis='y',
                stackratio = 0.02,
...
compare_dists_violin(all_w_dat, all_e_dat) 
ggsave("overall_distribution_violin.png", overall_distribution)
```
So yeah, the overal general distribution of scores is basically the same... Furthermore, the difference should be a uniform distribution... and it is. 

Maybe break it down by gestures?
```{r}
get_comparable_data <- function(dat, g_name) {
  wdat <- dat %>% 
    filter(viewer_culture=="western") %>%
    mutate(gesture=g_name)
  edat <- dat %>% 
    filter(viewer_culture=="eastern") %>%
    mutate(gesture=g_name)
...
wg1_compdat <- get_comparable_data(wg1_total, "wg1")
wg2_compdat <- get_comparable_data(wg2_total, "wg2")
wg3_compdat <- get_comparable_data(wg3_total, "wg3")
wg4_compdat <- get_comparable_data(wg4_total, "wg4")
wg5_compdat <- get_comparable_data(wg5_total, "wg5")

eg1_compdat <- get_comparable_data(eg1_total, "eg1")
eg2_compdat <- get_comparable_data(eg2_total, "eg2")
eg3_compdat <- get_comparable_data(eg3_total, "eg3")
eg4_compdat <- get_comparable_data(eg4_total, "eg4")

wg1_dist_plot <- compare_dists(wg1_compdat$w, wg1_compdat$e, "wg1")
wg2_dist_plot <- compare_dists(wg2_compdat$w, wg2_compdat$e, "wg2")
wg3_dist_plot <- compare_dists(wg3_compdat$w, wg3_compdat$e, "wg3")
wg4_dist_plot <- compare_dists(wg4_compdat$w, wg4_compdat$e, "wg4")
wg5_dist_plot <- compare_dists(wg5_compdat$w, wg5_compdat$e, "wg5")

eg1_dist_plot <- compare_dists(eg1_compdat$w, eg1_compdat$e, "eg1")
eg2_dist_plot <- compare_dists(eg2_compdat$w, eg2_compdat$e, "eg2")
eg3_dist_plot <- compare_dists(eg3_compdat$w, eg3_compdat$e, "eg3")
eg4_dist_plot <- compare_dists(eg4_compdat$w, eg4_compdat$e, "eg4")

grid.arrange(wg1_dist_plot, 
             wg2_dist_plot, 
             wg3_dist_plot, 
             wg4_dist_plot,
             eg1_dist_plot, 
...
```{r}
create_bucketed_bar_chart_western_only <- function(dat, gest) {
  d <- get_bucketed_data(dat) %>%
    filter(viewer_culture=="western")
  test_comparisons <- list(c("cond1","cond2"),c("cond1", "original"),c("cond2", "original"))
  p <- ggplot(d,aes(x=condition,y=bucket_percent,fill=factor(culture_bucket)))+
    geom_bar(stat="identity",position="dodge")+
    scale_fill_manual("Score Bucket", 
                      labels=c("Western 5-7", "Western 4", "Western 1-3"),
                      values=c("ahigh_western" = "palegreen3", # steelblue1, 3, 4
                               "bmedium_western" = "steelblue3",
                               "clow_western" = "palevioletred3")) +
    xlab("Gesture Condition")+ylab("Score bucket as proportional to total scores")+
    ggtitle(paste("Bucketed scores for", gest, sep=" ")) + 
    theme(text=element_text(size=12, 
                            family="serif")) + 
...
create_bucketed_bar_chart_eastern_only <- function(dat, gest) {
  d <- get_bucketed_data(dat) %>%
    filter(viewer_culture=="eastern")
  test_comparisons <- list(c("cond1","cond2"),c("cond1", "original"),c("cond2", "original"))
  p <- ggplot(d,aes(x=condition,y=bucket_percent,fill=factor(culture_bucket)))+
    geom_bar(stat="identity",position="dodge")+
    scale_fill_manual("Score Bucket", 
                      labels=c("Eastern 5-7", "Eastern 4", "Eastern 1-3"),
                      values=c("ahigh_eastern" = "palegreen3", # steelblue1, 3, 4
                               "bmedium_eastern" = "steelblue3",
                               "clow_eastern" = "palevioletred3")) +
    xlab("Gesture Condition")+ylab("Score bucket as proportional to total scores")+
    ggtitle(paste("Bucketed scores for", gest, sep=" ")) + 
    theme(text=element_text(size=12, 
                            family="serif")) + 
...
wg1_w_viewers_only_bar <- create_bucketed_bar_chart_western_only(wg1_total, "WG1")
wg1_e_viewers_only_bar <- create_bucketed_bar_chart_eastern_only(wg1_total, "WG1")
```

Test that significance (within-culture)
```{r}
test_significance_within_culture <- function(dat, culture="western") {
  d <- dat %>%
    filter(viewer_culture==culture)
  groups = c("certainty", "conflict", "control", "openness", "size", "unity")
  t <- c()
  cond2_1 <- c()
  sig_cond2_1 <- c()
  original_cond1 <- c()
  sig_original_cond1 <- c()
  original_cond2 <- c()
  sig_original_cond2 <- c()
  for(g in groups) {
    d_t <- d %>%
      filter(group==g)
    av <- aov(score ~ condition, data=d_t)
    tukey <- as.data.frame(TukeyHSD(av)$condition)
    cond2_1 <- list.append(cond2_1, tukey$`p adj`[1])
    sig_cond2_1 <- list.append(sig_cond2_1, get_sig(tukey$`p adj`[1]))
    original_cond1 <- list.append(original_cond1, tukey$`p adj`[2])
    sig_original_cond1 <- list.append(sig_original_cond1, get_sig(tukey$`p adj`[2]))
    original_cond2 <- list.append(original_cond2, tukey$`p adj`[3])
    sig_original_cond2 <- list.append(sig_original_cond2, get_sig(tukey$`p adj`[3]))
  }
  s <- d %>%
    group_by(group, condition) %>%
    summarise(avg=mean(score))
  
  s$cond1_2_diff <- cond2_1
  s$original_1_diff <- original_cond1
  s$original_2_diff <- original_cond2
  s$sig_cond2_1 <- sig_cond2_1
  s$sig_original_cond1 <- sig_original_cond1
  s$sig_original_cond2 <- sig_original_cond2
  
  return(s)
}

wg1_diff_w_viewers_only <- test_significance_within_culture(wg1_total, "western")
wg1_diff_e_viewers_only <- test_significance_within_culture(wg1_total, "eastern")

grid.arrange(wg1_w_viewers_only_bar, wg1_e_viewers_only_bar)
```
...
```{r}
wg1_w_diff <- wg1_diff_w_viewers_only %>%
  mutate(viewer_culture="western")
wg1_e_diff <- wg1_diff_e_viewers_only %>%
  mutate(viewer_culture="eastern")

wg1_viewer_within_diffs <- rbind(wg1_w_diff, wg1_e_diff)
```
...
## Just some regular old checks. 
Compare means of each culture in each group. 
```{r}
wg1.w.viewers <- wg1_total %>%
  filter(viewer_culture=="western") 
wg1.e.viewers <- wg1_total %>%
  filter(viewer_culture=="eastern") 

wg1.w.viewers$viewer_culture <- NULL
wg1.e.viewers$viewer_culture <- NULL

wg1.w.certainty0 <- wg1.w.viewers %>% filter(group=="certainty", condition=="original")
wg1.w.certainty1 <- wg1.w.viewers %>% filter(group=="certainty", condition=="cond1")
wg1.w.certainty2 <- wg1.w.viewers %>% filter(group=="certainty", condition=="cond2")

t.test(wg1.w.certainty0$score, wg1.w.certainty1$score)
t.test(wg1.w.certainty0$score, wg1.w.certainty2$score)

wg1.w.conflict <- wg1.w.viewers %>% filter(group=="conflict", condition=="original")

wg1.w.conflict0 <- wg1.w.viewers %>% filter(group=="conflict", condition=="original")
wg1.w.conflict1 <- wg1.w.viewers %>% filter(group=="conflict", condition=="cond1")
wg1.w.conflict2 <- wg1.w.viewers %>% filter(group=="conflict", condition=="cond2")

t.test(wg1.w.conflict0$score, wg1.w.conflict1$score)
t.test(wg1.w.conflict0$score, wg1.w.conflict2$score)




wg1.w.control <- wg1.w.viewers %>% filter(group=="control", condition=="original")
wg1.w.unity <- wg1.w.viewers %>% filter(group=="unity", condition=="original")
wg1.w.openness <- wg1.w.viewers %>% filter(group=="openness", condition=="original")
wg1.w.size <- wg1.w.viewers %>% filter(group=="size", condition=="original")

wg1.e.certainty <- wg1.e.viewers %>% filter(group=="certainty", condition=="original")
wg1.e.conflict <- wg1.e.viewers %>% filter(group=="conflict", condition=="original")
wg1.e.control <- wg1.e.viewers %>% filter(group=="control", condition=="original")
wg1.e.unity <- wg1.e.viewers %>% filter(group=="unity", condition=="original")
wg1.e.openness <- wg1.e.viewers %>% filter(group=="openness", condition=="original")
wg1.e.size <- wg1.e.viewers %>% filter(group=="size", condition=="original")

t.test(wg1.w.certainty$score, wg1.e.certainty$score)$p.value
t.test(wg1.w.conflict$score, wg1.e.conflict$score)
t.test(wg1.w.control$score, wg1.e.control$score)
t.test(wg1.w.unity$score, wg1.e.unity$score)
t.test(wg1.w.openness$score, wg1.e.openness$score)
t.test(wg1.w.size$score, wg1.e.size$score)
```

Compare the within-culture differences caused by the manipulations.
```{r}

get_within_culture_diffs_by_group <- function(g){
  dt <- data.frame(
    culture = rep(c("western", "eastern"), each=3),
    condition = rep(c("original", "cond1", "cond2"), times=2),
    group = rep(g, times=6),
    means = c(""),
    p.within.culture = c(""),
    stringsAsFactors=FALSE
  ...
  wg1.w.g0 <- wg1.w.viewers %>% filter(group==g, condition=="original")
  wg1.w.g1 <- wg1.w.viewers %>% filter(group==g, condition=="cond1")
  wg1.w.g2 <- wg1.w.viewers %>% filter(group==g, condition=="cond2")
  
  dt[1:3,]$means <- c(round(mean(wg1.w.g0$score), 2),
                      round(mean(wg1.w.g1$score), 2),
                      round(mean(wg1.w.g2$score), 2))
  
  dt[1:3,]$p.within.culture <- c(round(t.test(wg1.w.g0$score, wg1.w.g0$score)$p.value, 3),
                                 round(t.test(wg1.w.g0$score, wg1.w.g1$score)$p.value, 3),
                                 round(t.test(wg1.w.g0$score, wg1.w.g2$score)$p.value, 3))
  
  wg1.e.g0 <- wg1.e.viewers %>% filter(group==g, condition=="original")
  wg1.e.g1 <- wg1.e.viewers %>% filter(group==g, condition=="cond1")
  wg1.e.g2 <- wg1.e.viewers %>% filter(group==g, condition=="cond2")
  
  dt[4:6,]$means <- c(round(mean(wg1.e.g0$score), 3),
                      round(mean(wg1.e.g1$score), 3),
                      round(mean(wg1.e.g2$score), 3))
  
  dt[4:6,]$p.within.culture <- c(round(t.test(wg1.e.g0$score, wg1.e.g0$score)$p.value, 3),
                                 round(t.test(wg1.e.g0$score, wg1.e.g1$score)$p.value, 3),
                                 round(t.test(wg1.e.g0$score, wg1.e.g2$score)$p.value, 3))  
  return(dt)
}

get_all_within_culture_diffs <- function() {
  cot <- get_within_culture_diffs_by_group("control")
  con <- get_within_culture_diffs_by_group("conflict")
  cer <- get_within_culture_diffs_by_group("certainty")
  opn <- get_within_culture_diffs_by_group("openness")
  siz <- get_within_culture_diffs_by_group("size")
  uni <- get_within_culture_diffs_by_group("unity")
  ret <- rbind(cer, con, cot, opn, siz, uni)
  return(ret)
}

c <- get_all_within_culture_diffs()

c$condition <- factor(c$condition, levels = c$condition)
c$culture <- factor(c$culture, levels = c$culture)

g <- ggplot(c, aes(y=means, x=culture)) + 
...
Ok ok ok I got it. Table to look like
viewer_culture |  original.mean   |   group    |   condition1  | condition 2 |  p.diff.1  | p.diff.2
  w                    4.92             cert          5.17          4.62          0.284       0.259
  e                    4.27             cert          3.775         4.18          0.05        0.739

but also need to check directionality of differences
```{r}
get_within_culture_diffs_by_group2 <- function(dat, g){
  wg1.w.viewers <- dat %>%
    filter(viewer_culture=="western") 
  wg1.e.viewers <- dat %>%
    filter(viewer_culture=="eastern") 
  ...
  dt <- data.frame(
    viewer_culture = c("western", "eastern"),
    original.mean = c(round(mean(wg1.w.g0$score), 3), round(mean(wg1.e.g0$score), 3)),
    group = rep(g, times=2),
    condition.1.mean = c(round(mean(wg1.w.g1$score), 3), round(mean(wg1.e.g1$score), 3)),
    condition.2.mean = c(round(mean(wg1.w.g2$score), 3), round(mean(wg1.e.g2$score), 3)),
    p.1 = c(round(t.test(wg1.w.g0$score, wg1.w.g1$score)$p.value, 3),
            round(t.test(wg1.w.g0$score, wg1.w.g2$score)$p.value, 3)),
    p.2 = c(round(t.test(wg1.e.g0$score, wg1.e.g1$score)$p.value, 3),
                                 round(t.test(wg1.e.g0$score, wg1.e.g2$score)$p.value, 3)),
    stringsAsFactors=FALSE
  )
  
  dt <- dt %>%
    mutate(p1.sig.diff=ifelse(p.1 <= 0.05, '*', ' ')) %>%
    mutate(p1.diff=ifelse(condition.1.mean <= original.mean, "v", "^")) %>%
    mutate(p2.sig.diff=ifelse(p.2 <= 0.05, '*', ' ')) %>%
    mutate(p2.diff=ifelse(condition.2.mean <= original.mean, "v", "^")) 
  
  return(dt)
}

get_all_within_culture_diffs2 <- function(dat, name) {
  cot <- get_within_culture_diffs_by_group2(dat, "control")
  con <- get_within_culture_diffs_by_group2(dat, "conflict")
  cer <- get_within_culture_diffs_by_group2(dat, "certainty")
  opn <- get_within_culture_diffs_by_group2(dat, "openness")
  siz <- get_within_culture_diffs_by_group2(dat, "size")
  uni <- get_within_culture_diffs_by_group2(dat, "unity")
  ret <- rbind(cer, con, cot, opn, siz, uni) %>%
    mutate("gesture_name" = name)
  return(ret)
}

compare_manipulations_between_cultures <- rbind( get_all_within_culture_diffs2(wg1_total, "wg1"), 
                                                 get_all_within_culture_diffs2(wg2_total, "wg2"), 
                                                 get_all_within_culture_diffs2(wg3_total, "wg3"), 
                                                 get_all_within_culture_diffs2(wg4_total, "wg4"), 
                                                 get_all_within_culture_diffs2(wg5_total, "wg5"), 
                                                 get_all_within_culture_diffs2(eg1_total, "eg1"), 
                                                 get_all_within_culture_diffs2(eg2_total, "eg2"), 
                                                 get_all_within_culture_diffs2(eg3_total, "eg3"), 
                                                 get_all_within_culture_diffs2(eg4_total, "eg4"))
```

Hey now this is an interesting table!! Shows that our manipulations only occasionally caused significant differences from the original gesture, and sometimes even that was in opposite directions!! 