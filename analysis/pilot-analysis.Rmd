...
get_pie_data <- function(dat, group1, group2) {
  ...
  mutate(conflict = Answer.conflict + Answer.tension, 
         ...
         confidence = Answer.confident + Answer.sure,
         control = Answer.dominant + Answer.control,
         size = Answer.many + Answer.members) %>%
  select(conflict, openness, unity, confidence, control, size)
  ...
  df <- data.frame(
    group = c("both", group1, group2, "neither"),
...
p_wg1.0 <- make_pie_chart(get_pie_data(wg1.0.w, "conflict", "unity"), "likert", "conflict/unity", "unpolite", "original")
p_wg1.1 <- make_pie_chart(get_pie_data(wg1.1.w, "conflict", "unity"), "likert", "conflict/unity", "unpolite", "intertwine")
p_wg1.2 <- make_pie_chart(get_pie_data(wg1.2.w, "conflict", "unity"), "likert", "conflict/unity", "unpolite", "collide")


```

```{r}
grid.arrange(p_wg1.0, p_wg1.1, p_wg1.2, ncol=1)
```
...
grid.arrange(p_wg1.0, p_wg1.0_ranked, p_wg1.1, p_wg1.1_ranked, p_wg1.2, p_wg1.2_ranked, nrow=3, ncol=2)
```
