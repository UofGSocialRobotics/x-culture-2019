...
violin_compare_cultures_across_meta_groups <- function(data, cond="original", gesture_name="unknown") {
  g <- ggplot(filter(data)) + 
    aes(x=condition,y=score,fill=viewer_culture) +
    geom_violin(alpha=0.1,position="identity", adjust=1) +
    #scale_fill_brewer(palette="PiYG") +
    scale_fill_manual(values=c("palevioletred3", "steelblue3")) +
    facet_wrap("group") +
    geom_dotplot(binaxis='y', position="identity", stackdir='center', dotsize=0.9, stackratio=0.25, alpha=0.5) +
    labs(
...
```{r combine_violin_plots}
g <- wg1_violin_culture_comparisons$c0_violin_comparison + 
