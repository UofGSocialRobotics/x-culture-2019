...
all_unpolite_data <- do_the_thing(wg1.0.w, wg1.1.w, wg1.2.w, "original", "intertwine", "collide")
all_entity_data <- do_the_thing(wg2.0.w, wg2.1.w, wg2.2.w)
all_audience_data <- do_the_thing(wg3.0.w, wg3.1.w, wg3.2.w)
all_protected_data <- do_the_thing(wg4.0.w, wg4.1.w, wg4.2.w)
all_bowl_data <- do_the_thing(wg5.0.w, wg5.1.w, wg5.2.w)

all_correlational_data <- plot_diff_matrix_pretty(rbind(create_difference_matrix(wg1.0.w), create_difference_matrix(wg1.1.w), create_difference_matrix(wg1.2.w),
                                                        create_difference_matrix(wg2.0.w), create_difference_matrix(wg2.1.w), create_difference_matrix(wg2.2.w),
                                                        create_difference_matrix(wg4.0.w), create_difference_matrix(wg4.1.w), create_difference_matrix(wg4.2.w),
                                                        create_difference_matrix(wg5.0.w), create_difference_matrix(wg5.1.w), create_difference_matrix(wg5.2.w),
                                                        create_difference_matrix(wg3.0.w), create_difference_matrix(wg3.1.w), create_difference_matrix(wg3.2.w)), 
...
do_all_culture_anovas <- function() {
  print("!!!!WG1!!!!!")
  do_anova_per_gest_by_culture(wg1_total, "original")
  do_anova_per_gest_by_culture(wg1_total, "intertwine")
  do_anova_per_gest_by_culture(wg1_total, "collide")
  print(" ")
}
...
make_table_per_gesturecondition_by_culture <- function(dat) {
  ...
  for(g in groups) {
    ...
    p_val <- t.test(w_only$score, e_only$score)[3]$p.value
    p_val <- round(p.adjust(p_val, method="bonferroni", n=12), digits=5)
    p <- append(p, p_val)
    sig <- append(sig,
                  get_sig_val(p_val))
  }

  df <- data.frame(group, western_mean, eastern_mean, p, sig)   
...
do_t_culture_groups <- function(dat, cond1="cond1", cond2="cond2", gesture_name="unknown") {
  c0 <- make_table_per_gesture_by_culture(filter(dat, condition=="original"))
  
  c1 <- make_table_per_gesture_by_culture(filter(dat, condition==cond1))
  
  c2 <- make_table_per_gesture_by_culture(filter(dat, condition==cond2))
  return_list <- list("c0_comparison"=c0, 
                      "c1_comparison"=c1,
                      "c2_comparison"=c2)
}

wg1_total_comparison_tables <- do_t_culture_groups(wg1_total, "intertwine", "collide", "wg1")
```
